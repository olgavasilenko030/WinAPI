...
CONST COLORREF g_WINDOWS_BACKGROUND[] = { RGB(0,0,150), RGB(75,75,75) };
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		(
			...
			0,//Escapement-Наклон шрифта в десятках градусах
			0,//Orientation
		...
		for (int i = 0; i < 4; i++)
		{
			HWND hButton_Operations = CreateWindowEx
			(
			...
			HBITMAP hBitmap_Operations = (HBITMAP)LoadImage
			(
				GetModuleHandle(NULL),
				"ButtonBMP\\metal_mistral\\button_plus.bmp" + i,
				IMAGE_BITMAP,
				g_i_BUTTON_SIZE, g_i_BUTTON_SIZE,
				LR_LOADFROMFILE
			);
			SendMessage(hButton_Operations, BM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBitmap_Operations);
		}
		...
		HBITMAP hBitmap_Clear = (HBITMAP)LoadImage
		(
			GetModuleHandle(NULL),
	...
	case WM_COMMAND:
	{
		...
		{
			//TODO:Если первый символ ".", то последний символ неправильный
